...
DROP TRIGGER upToadmin;
DROP TRIGGER upToArtist;
DROP TRIGGER degradeToCommon;

DELIMITER //
CREATE TRIGGER upToadmin AFTER INSERT ON ADMINISTRATOR FOR EACH ROW
    BEGIN
        UPDATE GENERICUSER SET rol = 'admin' WHERE code = NEW.code;
        DELETE FROM COMMONUSER WHERE code = NEW.code;
    END;
//
DELIMITER ;

DELIMITER //
CREATE TRIGGER upToArtist AFTER INSERT ON ARTIST FOR EACH ROW
    BEGIN
        UPDATE GENERICUSER SET rol = 'artist' WHERE code = NEW.code;
        DELETE FROM COMMONUSER WHERE code = NEW.code;
    END;
//
DELIMITER ;
...
DELIMITER //
CREATE TRIGGER degradeToCommon AFTER DELETE ON ARTIST FOR EACH ROW
    BEGIN
        UPDATE GENERICUSER SET rol = 'common' WHERE code = OLD.code;
    END;
//
DELIMITER ;